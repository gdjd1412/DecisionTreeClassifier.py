...
df.describe()
print(df.describe())
with open("葡萄各参数指标的描述性统计结果.txt", 'w') as f:
...
plt.figure(figsize=(20, 10)) #利用seaborn库绘制红酒不同参数图
...
# 仙粉黛(Zinfandel)，西拉（Syrah），赤霞珠(Sauvignon)
res = model.predict(X_test)
name_map = {'Zinfandel':'仙粉黛', 'Syrah':'西拉', 'Sauvignon':'赤霞珠'}
res = [name_map[i] for i in res]
print('\n\n\n训练集预测结果：')#输出测试的结果，即输出模型预测的结果
print(res)
